...
EOF
    ...
    while true; do
        printf "o3din> Enter WhatsApp number (without +): "
    ...
    printf "o3din> Enter pairing code [default: CUMICUMI]: "
...
EOF
    ...
    printf "o3din> Add owner numbers? [y/N]: "
    read -r add_owners < /dev/tty
    
    if [[ "$add_owners" =~ ^[Yy]$ ]]; then
        OWNER_LIST=()
        while true; do
            printf "o3din> Owner number (without + or blank to finish): "
